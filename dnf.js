...
,
    {t:"体验服", v:"7016761", opt_data_array:[

        {t: "体验一区",v: "99",status:"1", display:"1", opt_data_array:[]}
	,
        {t: "体验二区",v: "199",status:"1", display:"1", opt_data_array:[]}
	...
	DNF.prototype = {
		...
		LoadAreaMap1:function(){ //大区
			_return = {}
			for(i in GameSever){
				_return[i] = GameSever[i].t;
			}
			return _return;
		},
		LoadAreaMap2:function(index){ //小区
			_return = {}
			for(i in GameSever[index].opt_data_array){
					_return[GameSever[index].opt_data_array[i].v] = GameSever[index].opt_data_array[i].t;
			
			}
			return _return;
		},
		QueryRole:function(area_id){ //角色
			_e = this;
			$.ajax({
				type: 'GET',
				url:"http://comm.aci.game.qq.com:8848/main?game=dnf&area="+area_id+"&callback=0&sCloudApiName=ams.gameattr.role&iAmsActivityId=54842&sServiceDepartment=xinyue",
				success: function(data,status,xhr){
					eval(data);
					_map = getQueryString(query_role_result_0.data,"_webplat_msg").split("|");
					for(i=1;i<_map.length-1;i++){
						//_tarr = {};
						_t = _map[i].split(" ");
						_t[0]; //角色id
						decodeURI(_t[1]); //角色名
						_e.RoleMap[_t[0]] = decodeURI(_t[1]);
